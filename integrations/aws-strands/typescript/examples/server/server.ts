...
// State streams naturally via the tool's async-generator yields, which the
// adapter translates into STATE_SNAPSHOT. No customResultHandler, no
// stopStreamingAfterResult, no direct AG-UI event emission.
...
/* ---------------- human-in-the-loop ---------------- */
// No backend stub — the frontend declares generate_task_steps via
// useHumanInTheLoop and the adapter auto-registers it as a proxy tool
// when RunAgentInput.tools arrives.
...
app.listen(port, () => {
  console.log(`TS strands server listening on :${port}`);
});
