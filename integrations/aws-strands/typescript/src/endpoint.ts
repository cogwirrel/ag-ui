 ...
 * emitted by this adapter (e.g. `ACTIVITY_*`, `RAW`, `REASONING_MESSAGE_CHUNK`)
 * or not yet wired (e.g. interrupts, resumable streams).
 *
...
export interface StrandsAguiCapabilities {
  ...
  features: {
    /** RunFinished.outcome interrupt + RunAgentInput.resume loop. Unsupported. */
    interrupts: boolean;
    /** Tool-call interrupts accept editedArgs. Unsupported. */
...
export const DEFAULT_CAPABILITIES: StrandsAguiCapabilities = {
  ...
  features: {
    interrupts: false,
    toolCallInterruptEditedArgs: false,
