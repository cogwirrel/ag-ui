...
export interface CreateStrandsAppOptions {
  ...
  /** Override capabilities advertised at {@link capabilitiesPath}. */
  capabilities?: import("./endpoint").StrandsAguiCapabilitiesOverrides;
...
export async function createStrandsApp(
  ...
  if (capabilitiesPath) {
    // Derive capabilities from the concrete agent so chunk-event flags track
    // its `emitChunkEvents` config automatically.
