/**
 * Verifies the adapter prefers `template.clone({...})` when the template Agent
 * exposes a `clone` method (Strands SDK with `Agent.clone`).
 *
 * The legacy slice-and-rebuild path is exercised by
 * `agent-config-forwarded.test.ts` and `plugins-forwarded.test.ts`, which mock
 * the SDK with an Agent stub that has no `clone` method.
 */
...
vi.mock("@strands-agents/sdk", async (importOriginal) => {
  ...
  /**
   * Stub Agent with a real `clone` method. Tracks every clone() call so the
   * adapter test can assert on the options shape.
   */
  class CloneCapableAgent {
    ...
    clone(options?: unknown): CloneCapableAgent {
      ...
      // Naive merge — the production clone() lives in the SDK; this stub just
      // needs to return a fresh instance the adapter can stream against.
      const overrides =
...
describe("StrandsAgent uses template.clone() when available", () => {
  ...
  it("invokes clone() per distinct threadId", async () => {
    capturedClones.length = 0;
    const sa = new StrandsAgent({ agent: template(), name: "agui" });
    await collect(sa, minimalRunInput({ threadId: "t1", runId: "r1" }));
    await collect(sa, minimalRunInput({ threadId: "t2", runId: "r2" }));
  ...
  it("resolves Plugin factories per thread so each clone gets a fresh instance", async () => {
    capturedClones.length = 0;
    const factory = vi.fn(
      () => ({ name: "p", initAgent: () => {} }) satisfies Plugin,
    );
    const sa = new StrandsAgent({
      agent: template(),
      name: "agui",
