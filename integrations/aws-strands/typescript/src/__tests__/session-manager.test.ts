...
import { Agent as StrandsAgentCore, SessionManager } from "@strands-agents/sdk";
...
describe("Session manager provider — caching", () => {
  ...
  it("different threads get separate provider invocations", async () => {
    ...
    await collect(minimalRunInput({ threadId: "thread-1" }), agent);
    await collect(minimalRunInput({ threadId: "thread-2" }), agent);
  ...
  it("failed provider does not cache — allows retry", async () => {
    ...
    expect(
      events1.some(
        (e) => (e as { code?: string }).code === "SESSION_MANAGER_ERROR",
      ),
    ).toBe(true);
...
describe("Session manager provider — empty/falsy threadId", () => {
  it("uses 'default' key when threadId is empty string", async () => {
    ...
    await collect(minimalRunInput({ threadId: "" }), agent);
    await collect(minimalRunInput({ threadId: "" }), agent);
...
describe("Session manager provider — strict instanceof validation", () => {
  it("rejects plain string with SESSION_MANAGER_INVALID_TYPE", async () => {
    ...
    expect((err as { code: string }).code).toBe("SESSION_MANAGER_INVALID_TYPE");
  });
  ...
  it("accepts a SessionManager subclass instance", async () => {
    ...
    expect(
      events.some((e) => (e as { type: string }).type === EventType.RUN_ERROR),
    ).toBe(false);
    expect(
      events.some(
        (e) => (e as { type: string }).type === EventType.RUN_FINISHED,
