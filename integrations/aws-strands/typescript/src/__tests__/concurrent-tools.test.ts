...
async function drain(agent: StrandsAgent) {
  ...
  for await (const e of agent.run(minimalRunInput())) {
    out.push(e as { type: string } & Record<string, unknown>);
  }
  return out;
}
...
describe("Concurrent tool executor envelope integrity (proof 4)", () => {
  it("interleaved two-tool stream preserves per-id START → ARGS → END → RESULT ordering", async () => {
    ...
    const out = await drain(new Testable(scriptedAgent(events)));
    ...
    const resA = out.find(
      (e) =>
        e.type === EventType.TOOL_CALL_RESULT && e.toolCallId === "strands-A",
    ) as unknown as { content: string };
    const resB = out.find(
      (e) =>
        e.type === EventType.TOOL_CALL_RESULT && e.toolCallId === "strands-B",
    ) as unknown as { content: string };
