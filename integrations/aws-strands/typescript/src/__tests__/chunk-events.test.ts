...
async function drain(agent: StrandsAgent): Promise<BaseEvent[]> {
  const out: BaseEvent[] = [];
...
describe("emitChunkEvents collapse", () => {
  ...
  it("default emits triples, no chunk events", async () => {
    const out = await drain(new ScriptedAgent(scripted, false));
  ...
  it("emitChunkEvents collapses TEXT/TOOL/REASONING triples into chunks, dropping *_END", async () => {
    const out = await drain(new ScriptedAgent(scripted, true));
    const types = out.map((e) => e.type);
