 ...
 *
 * Prefer the model ID string over the model instance. Sharing a Model object
 * between agents causes ``structuredClone`` failures in the AWS SDK's JSON
 * serializer when tool specs contain native function refs.
 */
...
export class StrandsAgent {
  ...
  constructor(options: StrandsAgentOptions) {
    ...
    // Detect the common footgun: sessionManager set on the template Agent
  ...
  async *run(inputData: RunAgentInput): AsyncGenerator<BaseEvent, void, void> {
    // interrupts.mdx rule 4: a resume[] entry pointing at an interruptId the
    // server has no record of MUST produce RUN_ERROR. This adapter doesn't
    // implement the interrupt/resume loop, so every `resume[]` entry is
    // "unknown" by definition. Gated above `_runRaw` so subclasses that
    // override only `_runRaw` still inherit the check.
    if (Array.isArray(inputData.resume) && inputData.resume.length > 0) {
      yield _runStarted(inputData);
      const ids = inputData.resume
        .map((entry) => entry.interruptId)
        .slice(0, 4);
      yield _runError(
        `This agent did not issue any interrupts to resume: ${ids.join(", ")}. ` +
  ...
  private async *_runSingleAgent(
    ...
    try {
      ...
      let invokeArgs: string | ContentBlock[] | undefined = userMessage;
      if (replayHistory) {
    ...
    try {
      ...
      for await (const rawEvent of this._orchestrator!.stream(prompt)) {
        ...
        if (kind === "multiAgentHandoffEvent") {
          const ev = event as { source?: string; targets?: string[]; message?: string };
          yield {
            type: EventType.CUSTOM,
            name: "MultiAgentHandoff",
            value: {
