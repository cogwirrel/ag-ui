 ...
 * compatible with SDK builds whose clone-options shape predates the unified
 * `overrides` field. Detection of clone() support is structural (see
 * `_getClone`).
 */
...
/**
 * Returns the bound `clone` method when the template exposes one, otherwise
 * `undefined`. This lets the adapter cleanly support older
 * `@strands-agents/sdk` versions without `Agent.clone` — they fall through to
 * the legacy slice-and-rebuild path in `_buildThreadAgent`.
 */
...
export class StrandsAgent {
  ...
  private _buildThreadAgent(
    ...
    const resolvedPlugins = this._plugins.map((p) =>
    ...
    if (cloneFn) {
      ...
      if (resolvedPlugins.length > 0) opts.additionalPlugins = resolvedPlugins;
      return cloneFn(opts);
    }

    // Legacy fallback for SDKs without `Agent.clone`. Uses the publicly
    // visible template surface only; unconnected MCP clients passed via the
    // template's `tools` cannot be recovered here because Strands routes them
    // into a private `_mcpClients` field at construction time.
    const t = template as unknown as {
      ...
      description?: string;
      id?: string;
      appState?: { getAll: () => Record<string, JSONValue> };
      modelState?: { getAll: () => Record<string, JSONValue> };
    ...
    const appStateDump = t.appState?.getAll();
    if (appStateDump && Object.keys(appStateDump).length > 0)
      cfg.appState = appStateDump as AgentConfig["appState"];
    const modelStateDump = t.modelState?.getAll();
    if (modelStateDump && Object.keys(modelStateDump).length > 0)
      cfg.modelState = modelStateDump as AgentConfig["modelState"];
    ...
    // default plugins".
    if (resolvedPlugins.length > 0) cfg.plugins = resolvedPlugins;
