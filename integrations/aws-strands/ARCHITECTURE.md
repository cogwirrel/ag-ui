...
- **Interrupt-rule gate**: per `interrupts.mdx` rule 4, any `RunAgentInput.resume[]` entry references an unknown interrupt by definition (this adapter doesn't emit interrupts). The adapter emits `RUN_STARTED` then `RUN_ERROR { code: "UNKNOWN_INTERRUPT" }`. Python currently ignores `resume[]` — tracked as a conformance gap.
...
The TypeScript package ships the same seven examples under the matching filenames (`agentic-chat.ts`, `agentic-chat-reasoning.ts`, `agentic-chat-multimodal.ts`, `backend-tool-rendering.ts`, `shared-state.ts`, `agentic-generative-ui.ts`, `human-in-the-loop.ts`). Each file is self-contained and can be run standalone (`pnpm <name>` from `examples/`). `examples/server/server.ts` is a "dojo" that mounts all seven at the paths the Python reference server uses, so both implementations can be driven by the same curl payloads.
